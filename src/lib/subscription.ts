...
/**
 * Downgrade user to FREE plan (no data deletion)
 */
export async function downgradeToFree(professionalId: string): Promise<boolean> {
  try {
    ...
    // Plan limits enforced by UI/backend filtering, not by data deletion
...
// REMOVED: deactivateExcessStaffMembers function
// Masters are never deleted or deactivated based on plan limits
// Plan limits are enforced through UI visibility and backend filtering only
